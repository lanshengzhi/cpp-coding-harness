...
TEST_CASE("limit_output passes input under the limits through unchanged", "[harness][output-limiter][issue66]") {
    ...
    const auto without_newline = harness::limit_output("line1\nline2");
...
TEST_CASE("limit_output oversized-first-line fallback never splits a multibyte sequence", "[harness][output-limiter][issue66]") {
    ...
    const auto two_byte = harness::limit_output(
        utf8_accented,
        harness::OutputLimit{.max_bytes = 4, .max_lines = 2000});
    ...
    const auto four_byte = harness::limit_output(
        emojis,
        harness::OutputLimit{.max_bytes = 5, .max_lines = 2000});
