...
[[nodiscard]] support::AsyncResult<int, E> post_result(
    ...
    std::expected<int, E> result) {
    return support::AsyncResult<int, E>{
        support::AsyncProducer<int, E>{
            [target, result = std::move(result)](support::AsyncCompletion<int, E> completion) mutable noexcept {
                const auto completion_owner =
                    std::make_shared<support::AsyncCompletion<int, E>>(std::move(completion));
                const auto result_owner = std::make_shared<std::expected<int, E>>(std::move(result));
                boost::asio::post(
                    target->executor(),
                    [completion_owner, result_owner]() mutable noexcept {
...
bool timeout_child_fails() {
    ...
    if (pid == 0) {
        ...
        support::AsyncResult<int> stalled{
            support::AsyncProducer<int, support::Error>{
...
public:
    explicit RecordingSession(std::vector<std::string>& events)
        : events_(&events) {}
...
TEST_CASE(
    "RuntimeFixture preserves ready AsyncResult success and error outcomes",
    "[coding_agent][runtime][harness][issue568]") {
    tests::RuntimeFixture runtime;

    const auto success = runtime.run(support::AsyncResult<int>{
        std::expected<int, support::Error>{42}});
    REQUIRE(success);
    CHECK(*success == 42);

    const auto error = runtime.run(support::AsyncResult<int>{
        std::expected<int, support::Error>{
            std::unexpect,
            support::make_error(support::ErrorCode::Process, "ready failure")}});
    REQUIRE_FALSE(error);
...
TEST_CASE(
    "RuntimeFixture preserves pending AsyncResult success and error outcomes",
    "[coding_agent][runtime][harness][issue568]") {
    tests::RuntimeFixture runtime;
    const auto target = runtime.make_target();

    const auto success = runtime.run(post_result(
        target,
        std::expected<int, support::Error>{23}));
    REQUIRE(success);
    CHECK(*success == 23);

    const auto error = runtime.run(post_result(
        target,
        std::expected<int, support::Error>{
            std::unexpect,
            support::make_error(support::ErrorCode::Cancelled, "pending cancellation")}));
    REQUIRE_FALSE(error);
...
TEST_CASE(
    "RuntimeFixture bounds a stalled AsyncResult as a test failure",
    "[coding_agent][runtime][harness][issue568]") {
...
TEST_CASE(
    "RuntimeFixture instances keep Runtime targets isolated",
    "[coding_agent][runtime][harness][issue568]") {
    tests::RuntimeFixture first;
    tests::RuntimeFixture second;

    const auto first_result = first.run(post_result(
        first.make_target(),
        std::expected<int, support::Error>{1}));
    const auto second_result = second.run(post_result(
        second.make_target(),
        std::expected<int, support::Error>{2}));
...
TEST_CASE(
    "multiple Agent Sessions share one RuntimeFixture and close before RuntimeRoot",
    "[coding_agent][runtime][harness][issue568]") {
    SharedSessionFixture fixture;

    auto first_result = fixture.runtime.run(coding_agent::create_agent_session_async(
        fixture.request(),
        std::nullopt,
        {}));
    REQUIRE(first_result);
    ...
    auto second_result = fixture.runtime.run(coding_agent::create_agent_session_async(
        fixture.request(),
        std::nullopt,
        {}));
    REQUIRE(second_result);
    ...
    CHECK(fixture.runtime.teardown_events().size() == 6);
...
TEST_CASE(
    "RuntimeFixture records Session destruction before Runtime worker join",
    "[coding_agent][runtime][harness][issue568]") {
