...
TEST_CASE("Native TUI exit during an admitted manual compaction waits for the compaction to settle",
    ...
    // settings the Session factory loads (through the isolated HOME) so the small
    ...
    config.write(".config/pike/agent/settings.json", R"({"compaction": {"keepRecentTokens": 1}})");
    const tests::EnvVarGuard agent_dir{"HOME", config.path().string()};
...
TEST_CASE("Native TUI command autocomplete offers every accepted slash spelling",
    ...
    REQUIRE(terminal.inject_input("\r"));
    drain_ready(io);
    // The session's initial System Message remains; `/clear` adds no Agent
