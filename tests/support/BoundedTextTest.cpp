...
TEST_CASE("bounded_utf8 never splits a multibyte sequence", "[support][bounded-text][issue66]") {
    ...
    CHECK(support::bounded_utf8(text, 3) == "a\xc3\xa9");
    CHECK(support::bounded_utf8(text, 4) == "a\xc3\xa9"
                                            "b");

    const std::string emoji = "\xf0\x9f\x99\x82"; // U+1F642
    CHECK(support::bounded_utf8(emoji, 3).empty());
...
TEST_CASE("bounded_utf8 replaces invalid sequences with U+FFFD", "[support][bounded-text][issue66]") {
    ...
    CHECK(support::bounded_utf8("\xed\xa0\x80", 16) == "\xef\xbf\xbd\xef\xbf\xbd\xef\xbf\xbd");
