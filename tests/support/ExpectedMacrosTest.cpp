...
boost::asio::awaitable<support::Expected<int>> success_int() {
    co_return 42;
}

boost::asio::awaitable<support::Expected<int>> failure_int() {
    co_return std::unexpected(support::make_error(support::ErrorCode::Validation, "int failure"));
}

boost::asio::awaitable<support::ExpectedVoid> success_void() {
    co_return support::ExpectedVoid{};
}

boost::asio::awaitable<support::ExpectedVoid> failure_void() {
    co_return std::unexpected(support::make_error(support::ErrorCode::Validation, "void failure"));
}

boost::asio::awaitable<support::Expected<int>> try_success() {
    CCH_TRY(value, co_await success_int());
    co_return value * 2;
}

boost::asio::awaitable<support::Expected<int>> try_failure() {
    CCH_TRY(value, co_await failure_int());
    co_return value * 2;
}

boost::asio::awaitable<support::Expected<int>> try_void_success() {
    CCH_TRY_VOID(co_await success_void());
    co_return 100;
}

boost::asio::awaitable<support::Expected<int>> try_void_failure() {
    CCH_TRY_VOID(co_await failure_void());
    co_return 100;
}

boost::asio::awaitable<support::Expected<int>> multiple_try_same_scope() {
    CCH_TRY(a, co_await success_int());
    CCH_TRY(b, co_await success_int());
    co_return a + b;
}

boost::asio::awaitable<support::Expected<int>> try_moved_value() {
    CCH_TRY(value, co_await success_int());
