...
TEST_CASE("normalized_secret_key strips non-alphanumerics and uppercases", "[support][redactor][issue66][spec]") {
...
TEST_CASE("looks_secret_key matches secret-shaped keys and exempts token counters",
        "[support][redactor][issue66][spec]") {
...
TEST_CASE("redact_text redacts colon-separated secret assignments", "[support][redactor][issue66][spec]") {
...
TEST_CASE("redact_text redacts equals-separated secret assignments", "[support][redactor][issue66][spec]") {
    CHECK(support::redact_text("PASSWORD=hunter2") == "PASSWORD=[REDACTED]");
    CHECK(support::redact_text("export API_TOKEN = abc123") == "export API_TOKEN = [REDACTED]");
}

TEST_CASE("redact_text leaves non-secret assignments untouched", "[support][redactor][issue66][spec]") {
...
TEST_CASE("redact_text redacts quoted values including escaped quotes", "[support][redactor][issue66][spec]") {
    CHECK(support::redact_text(R"(token = "ab\"cd" rest)") == R"(token = "[REDACTED]" rest)");
    CHECK(support::redact_text("secret='plain'") == "secret='[REDACTED]'");
}

TEST_CASE("redact_text keeps authorization values with spaces together", "[support][redactor][issue66][spec]") {
    CHECK(support::redact_text("Authorization: Bearer abc def\nnext") == "Authorization: [REDACTED]\nnext");
}

TEST_CASE("redact_text stops unquoted non-authorization values at whitespace", "[support][redactor][issue66][spec]") {
    CHECK(support::redact_text("api_key=abc def") == "api_key=[REDACTED] def");
}

TEST_CASE("redact_text redacts sk- tokens with long enough suffixes", "[support][redactor][issue66][spec]") {
...
TEST_CASE("redact_text leaves short sk- tokens untouched", "[support][redactor][issue66][spec]") {
    CHECK(support::redact_text("code sk-abc end") == "code sk-abc end");
}

TEST_CASE("redact_text redacts AKIA tokens of uppercase digits only", "[support][redactor][issue66][spec]") {
...
TEST_CASE("redact_text redacts multiple secrets in one text", "[support][redactor][issue66][spec]") {
    CHECK(support::redact_text("a=sk-12345678 and password: x") == "a=[REDACTED] and password: [REDACTED]");
}

TEST_CASE("redact_text is idempotent", "[support][redactor][issue66][spec]") {
