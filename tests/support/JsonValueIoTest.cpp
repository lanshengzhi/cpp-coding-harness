...
TEST_CASE("JsonValue serializes numbers byte-identically to the Glaze path", "[support][json][t6]") {
    ...
    for (const auto& [value, expected] : cases) {
        const auto serialized = support::write_json(support::JsonValue{value});
        REQUIRE(serialized);
...
TEST_CASE("JsonValue serializes only the Glaze escape set, raw otherwise", "[support][json][t6]") {
    ...
    const auto slash = support::write_json(support::JsonValue{"/"});
    REQUIRE(slash);
    CHECK(*slash == "\"/\"");
}
...
TEST_CASE("JsonValue parser handles whitespace-only containers like Glaze", "[support][json][t6]") {
    ...
    for (const auto& [input, expected] : cases) {
        ...
        REQUIRE(parsed);
        const auto serialized = support::write_json(*parsed);
        REQUIRE(serialized);
        CHECK(*serialized == expected);
    }
}
...
TEST_CASE("JsonValue parser accepts trailing content like Glaze", "[support][json][t6]") {
    ...
    const auto parsed = support::read_json("{\"a\":1} {\"b\":2}");
    REQUIRE(parsed);
    const auto serialized = support::write_json(*parsed);
    REQUIRE(serialized);
...
TEST_CASE("JsonValue parser applies strict JSON grammar and Glaze tolerances", "[support][json][t6]") {
    ...
    for (const std::string input : {"", "   ", ".5", "5.", "007", "[1,2,]", "{\"a\":1,}",
                                    "\"abc", "\"\\q\"", "\"\\u12\"", "{\"a\":}", "{\"a\" 1}"}) {
        CHECK_FALSE(support::read_json(input));
    }
    // Numbers out of double range fail; duplicate keys keep the last value.
    CHECK_FALSE(support::read_json("1e999"));
    CHECK_FALSE(support::read_json("1e-999"));
    const auto duplicates = support::read_json(R"({"a":1,"a":2})");
    REQUIRE(duplicates);
    const auto serialized = support::write_json(*duplicates);
    REQUIRE(serialized);
