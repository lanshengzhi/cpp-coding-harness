...
TEST_CASE("visible_width measures CJK characters as width 2", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("visible_width measures emoji as width 2", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("visible_width handles combining marks as width 0", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("visible_width strips ANSI escape codes", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("visible_width handles OSC 8 hyperlinks", "[tui][issue46][unicode][spec]") {
    const std::string link = "\x1b]8;;https://example.com\x07link\x1b]8;;\x07";
    CHECK(visible_width(link) == 4);
}

TEST_CASE("visible_width handles a regional indicator pair as one flag", "[tui][issue46][unicode][spec]") {
    const std::string flag = "\xf0\x9f\x87\xa6\xf0\x9f\x87\xba";
    CHECK(visible_width(flag) == 2);
}

TEST_CASE("visible_width uses Unicode properties for symbols and combining marks", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("truncate_text truncates ASCII correctly", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("truncate_text passes through short text", "[tui][issue46][unicode][spec]") {
    auto r = truncate_text("hi", 10);
    REQUIRE(r);
    CHECK(*r == "hi");
}

TEST_CASE("truncate_text pads to width when requested", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("truncate_text handles CJK characters", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("truncate_text preserves ANSI styling at truncation boundary", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("wrap_text preserves control ordering around whitespace", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("wrap_text does not wrap short text", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("wrap_text preserves literal newlines", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("wrap_text handles CJK word wrapping", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("wrap_text rejects zero width", "[tui][issue46][unicode][spec]") {
    auto r = wrap_text("hello", 0);
    REQUIRE_FALSE(r);
}

TEST_CASE("slice_by_column extracts a plain ASCII range", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("slice_by_column carries leading styling into the slice", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("slice_by_column handles empty ranges and out-of-range starts", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("strip_terminal_sequences removes SGR and hyperlink codes", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("strip_terminal_sequences preserves visible text and newlines", "[tui][issue46][unicode][spec]") {
    CHECK(strip_terminal_sequences("a\x1b[31mb\x1b[0m\nc") == "ab\nc");
}

TEST_CASE("strip_terminal_sequences strips ST-terminated and unterminated sequences", "[tui][issue46][unicode][spec]") {
...
TEST_CASE("strip_terminal_sequences strips cursor-movement finals like pi", "[tui][issue46][unicode][spec]") {
