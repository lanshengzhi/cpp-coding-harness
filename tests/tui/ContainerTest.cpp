...
TEST_CASE("Container requires positive width", "[tui][issue46][container][spec]") {
    cch::tui::Container container;
...
TEST_CASE("Box bounds throwing and overwide background hooks", "[tui][issue46][container][spec]") {
...
TEST_CASE("Box rejects width too small for padding", "[tui][issue46][container][spec]") {
    cch::tui::Box box(2, 1);
    auto text = std::make_unique<cch::tui::Text>("hi", 0, 0);
    REQUIRE(box.add_child(std::move(text)));
...
TEST_CASE("Box clear removes all children", "[tui][issue46][container][spec]") {
    cch::tui::Box box;
    auto text = std::make_unique<cch::tui::Text>("hi", 0, 0);
    REQUIRE(box.add_child(std::move(text)));
    box.clear();
