...
TEST_CASE("TruncatedText passes through short text", "[tui][truncated][issue381][spec]") {
...
TEST_CASE("TruncatedText hard-cuts long text at the width boundary", "[tui][truncated][issue381][spec]") {
...
TEST_CASE("TruncatedText hard-cuts ANSI-bearing text retaining escapes", "[tui][truncated][issue381][spec]") {
...
TEST_CASE("TruncatedText only renders first line", "[tui][truncated][issue381][spec]") {
    cch::tui::TruncatedText text("hello\nworld");
    auto result = text.render(20);
    REQUIRE(result);
    REQUIRE(result->lines.size() >= 1);
...
TEST_CASE("TruncatedText applies padding", "[tui][truncated][issue381][spec]") {
...
TEST_CASE("TruncatedText rejects width consumed by padding", "[tui][truncated][issue381][spec]") {
...
TEST_CASE("TruncatedText rejects zero width", "[tui][truncated][issue381][spec]") {
