...
util::Expected<T> run_awaitable(Start start) {
    ...
    auto future = boost::asio::co_spawn(
        io,
        [start = std::move(start)]() mutable -> boost::asio::awaitable<util::Expected<T>> {
...
TEST_CASE("process runner drains both pipes beyond kernel buffer capacity", "[util][process][issue458]") {
...
wait)"};
    ...
    request.output_limit = util::OutputLimit{.max_bytes = 1024, .max_lines = 1000000};
...
TEST_CASE("process runner contains a throwing callback and keeps both pipes draining", "[util][process][issue458]") {
    util::DefaultAsyncProcessRunner runner;
    util::ProcessRequest request;
    request.executable = "/bin/bash";
    request.arguments = {"-c", R"((head -c 262144 /dev/zero | tr '\0' 'A') &
...
wait)"};
    ...
    CHECK(result.error().code == util::ErrorCode::Process);
...
TEST_CASE("process runner caps newline-free stderr output without stopping the drain", "[util][process][issue458]") {
    util::DefaultAsyncProcessRunner runner;
    util::ProcessRequest request;
    request.executable = "/bin/bash";
...
TEST_CASE("process runner truncates at the line limit and keeps draining to EOF", "[util][process][issue458]") {
    util::DefaultAsyncProcessRunner runner;
    util::ProcessRequest request;
    request.executable = "/bin/bash";
    ...
    request.working_directory = std::filesystem::current_path();
    request.timeout = std::chrono::milliseconds{5000};
    request.output_limit = util::OutputLimit{.max_bytes = 1024 * 1024, .max_lines = 100};
...
TEST_CASE("process runner timeout terminates and reaps the child", "[util][process][issue458]") {
#if defined(__unix__) || defined(__APPLE__)
...
TEST_CASE("process runner cancellation terminates and reaps the process group", "[util][process][issue458]") {
#if defined(__unix__) || defined(__APPLE__)
    tests::TempWorkspace workspace;
    const auto pid_file = (workspace.path() / "shell.pid").string();
    ...
    boost::asio::io_context io;
    std::optional<util::Expected<util::ProcessResult>> result;
    boost::asio::co_spawn(
        io,
...
TEST_CASE("process runner keeps stdout and stderr independent for mixed traffic", "[util][process][issue458]") {
    util::DefaultAsyncProcessRunner runner;
    util::ProcessRequest request;
    request.executable = "/bin/bash";
    ...
    request.working_directory = std::filesystem::current_path();
    request.timeout = std::chrono::milliseconds{5000};
    request.output_limit = util::OutputLimit{.max_bytes = 1024 * 1024, .max_lines = 100000};
