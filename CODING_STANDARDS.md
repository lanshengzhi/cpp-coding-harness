...
10.4. Filesystem path resolution lives in `src/agent/harness/WorkspaceFileSystem.hpp` and its split implementation units `WorkspaceFileSystem{FdWalk,Legacy,Pi,Temp}.cpp`. Every operation resolves uniformly through pi `resolveToCwd` semantics (ADR 0057): unicode-space normalization, leading-`@` stripping, and `~` expansion against `$HOME`, then absolute paths are honored anywhere after lexical normalization and relative paths resolve against the workspace root with `..` resolved by normalization rather than rejected. Filesystem access authority matches the host user process — there is no workspace containment, path allowlist, or resolution-scope split. The no-follow symlink policy and atomic writes apply uniformly. File tools route through it; flag a file-tool path that bypasses its resolution or duplicates its path checks elsewhere. Path handling for unrelated capabilities is outside this rule (`docs/agents/architecture.md` §Security and containment).
