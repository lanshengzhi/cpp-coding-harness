...
| 3 | Terminal-error-event contract: exactly one `error`/`aborted` terminal event plus a final `AssistantMessage` with agreeing `stopReason`/`errorMessage` (incl. terminal-before-start synthesized start) | #326; `packages/agent/src/agent-loop.ts` `streamAssistantResponse` (`done`/`error` → `message_start` synthesis) | `src/agent/AgentLoop.cpp` (synthesize-start + terminal turn), `tests/support/FakeModelStream.hpp` (exactly-one-terminal script) | `ModelRuntimeSeamTest` terminal/aborted/matrix rows; `AgentCoreEvidenceTest` → `loop-terminal-error.json`, `loop-terminal-aborted.json` |
