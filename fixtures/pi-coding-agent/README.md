...
| 32 | `/reload`: re-reads User Settings, keybindings, skills, templates, themes, and Project Context Files; rebuilds the System Prompt; refreshes loaded resources; refuses while streaming/compacting with pi's verbatim warnings | `pi:packages/coding-agent/src/modes/interactive/interactive-mode.ts` (`/reload` handler), `core/resource-loader.ts` | `src/coding_agent/runtime/AgentSessionRuntime.*` (`reload()`), `src/coding_agent/tui/ReloadBox.*` | `ResourceReloadTest` (`[coding_agent][reload][issue418]`), `LoadedResourcesTest`, `BuiltinSlashCommandsTest` (`/reload` rows) |
