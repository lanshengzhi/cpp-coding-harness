...
| D20 | `/export` `/import` `/share` `/changelog` `/clone` (user ruling), the hidden pi-only `/debug` developer entry, the easter eggs; `/bug` (pi's own bug-reporting endpoint: not-applicable, #793) | `pi:packages/coding-agent/src/core/slash-commands.ts` | G4 ([#392]), #793 | `BuiltinSlashCommandsTest` (absent from autocomplete, `bug` asserted too); `SlashCommandRouterTest`/`InteractiveModeTest` (typed Deferred names fall through as Agent Prompts, issue #792) |
...
| 3 | Login dialogs | The login dialog / OAuth selector for OAuth-capable paths (Codex/OpenRouter auth URL, device code, manual code, info, and waiting views) and the API-key dialog branches for Kimi/DeepSeek render and function | Manual run with a real (or deliberately failing) auth flow; no live credentials are ever committed |
