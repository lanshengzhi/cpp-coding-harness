...
[[nodiscard]] std::string resolve_oauth_host(const KimiCodingOAuthOptions& options) {
    ...
    } else if (const char* first = std::getenv("KIMI_CODE_OAUTH_HOST"); first != nullptr && *first != '\0') {
        host = first;
    } else if (const char* second = std::getenv("KIMI_OAUTH_HOST"); second != nullptr && *second != '\0') {
        host = second;
    }
    if (host.empty()) {
...
[[nodiscard]] bool trusted_http_url(std::string_view value) {
    ...
    if (scheme != "http" && scheme != "https" && scheme != "HTTP" && scheme != "HTTPS") {
        return false;
    }
...
public:
    RequestStopScope(std::stop_token parent, boost::asio::any_io_executor executor, std::chrono::milliseconds timeout)
    ...
    ~RequestStopScope() { timer_->cancel(); }
...
[[nodiscard]] boost::asio::awaitable<support::Expected<OAuthHttpResponse>> post_kimi_request(
    ...
    auto response = co_await http_client->post(std::move(url), kFormHeaders, std::move(body), scope.token());
    if (!response) {
        ...
        if (scope.timed_out()) {
            co_return std::unexpected(
                    support::make_error(support::ErrorCode::Timeout, "Kimi Code OAuth request timed out"));
        }
...
[[nodiscard]] support::Expected<OAuthToken> parse_token_response(
    ...
    const auto missing_fields = [&response, operation]() {
        return support::make_error(support::ErrorCode::OAuth,
                "Kimi Code token " + std::string{operation} + " response missing fields: " + response.body);
    };
...
[[nodiscard]] DevicePollResult<OAuthToken> poll_kimi_token(const OAuthHttpResponse& response) {
    ...
    if (response.status_code >= 200 && response.status_code < 300 && access) {
        if (auto parsed = parse_token_response(response, "poll"); parsed) {
            return DevicePollResult<OAuthToken>{
                    .kind =
                            DevicePollResult<OAuthToken>::Complete{
    ...
    if (*error == "slow_down") {
        ...
        return DevicePollResult<OAuthToken>{
                .kind =
                        DevicePollResult<OAuthToken>::SlowDown{
...
[[nodiscard]] bool is_retryable_refresh_failure(int status_code) { return status_code == 429 || status_code >= 500; }
...
[[nodiscard]] boost::asio::awaitable<support::Expected<DeviceAuthorization>> start_device_authorization(
    ...
    CCH_TRY(response,
            co_await post_kimi_request(http_client,
    ...
    if (response.status_code < 200 || response.status_code >= 300) {
        co_return std::unexpected(support::make_error(support::ErrorCode::OAuth,
                "Kimi Code device authorization failed with status " + std::to_string(response.status_code) +
                        (response.body.empty() ? "" : ": " + response.body)));
    }
    ...
    if (!device_code || !user_code || !verification_uri || !verification_uri_complete ||
        ...
        co_return std::unexpected(support::make_error(
                support::ErrorCode::OAuth, "Invalid Kimi Code device authorization response: " + response.body));
    }
...
[[nodiscard]] boost::asio::awaitable<support::Expected<OAuthToken>> poll_for_token(
        ...
        std::chrono::milliseconds request_timeout) {
    CCH_TRY(token,
            co_await poll_device_flow<OAuthToken>(DevicePollOptions<OAuthToken>{
                    .interval_seconds = device.interval_seconds,
                    .expires_in_seconds = device.expires_in_seconds,
                    .wait_before_first_poll = true,
                    .poll = [http_client, oauth_host, device, stop_token, request_timeout]()
                            -> boost::asio::awaitable<support::Expected<DevicePollResult<OAuthToken>>> {
                        const std::string body = "client_id=" + url_query_encode(kClientId) +
