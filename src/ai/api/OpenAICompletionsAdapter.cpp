...
[[nodiscard]] bool is_openrouter(const Model& model) {
    return model.provider == "openrouter" ||
           model.base_url.find("openrouter.ai") != std::string::npos;
}
...
[[nodiscard]] support::Expected<providers::StreamRequest> build_stream_request(
    ...
    request.headers.insert(
            options.auth.headers.begin(),
            options.auth.headers.end());
    if (is_openrouter(model) && options.session_id &&
            options.cache_retention != CacheRetention::None &&
            !has_header(request.headers, "x-session-id") &&
...
[[nodiscard]] support::ExpectedVoid process_sse_event(
    ...
    if (event.event == "error") {
        const auto provider_code =
                providers::provider_error_code_from_payload(event.data);
        inference_failure = InferenceFailure{
                .kind = provider_code
                        ? providers::inference_failure_kind_from_provider_code(
                                  *provider_code)
    ...
    if (!parsed) {
        return std::unexpected(providers::make_stream_error(
                "Malformed OpenAI Chat Completions SSE event",
                parsed.error().detail));
    }
...
OpenAICompletionsAdapter::OpenAICompletionsAdapter(
        std::shared_ptr<providers::StreamTransport> transport)
    : transport_(std::move(transport)) {}

OpenAICompletionsAdapter::OpenAICompletionsAdapter(
        OpenAICompletionsAdapter&&) noexcept = default;
OpenAICompletionsAdapter& OpenAICompletionsAdapter::operator=(
        OpenAICompletionsAdapter&&) noexcept = default;
...
OpenAICompletionsAdapter::stream(
        ...
        AssistantEventSink sink) {
    if (!transport_) {
        co_return std::unexpected(providers::make_stream_error(
                "OpenAI Chat Completions adapter requires a stream transport"));
    }
    if (model.api != "openai-completions") {
        co_return std::unexpected(providers::make_stream_error(
                "OpenAI Chat Completions adapter received the wrong Model API"));
    }
    if (model.base_url.empty()) {
        co_return std::unexpected(providers::make_stream_error(
                "OpenAI Chat Completions Model base URL is required"));
    }
    ...
    auto attempt_hook = [attempt_state, &model]()
            -> support::Expected<providers::SseEventHook> {
        attempt_state->processor =
                std::make_unique<CompletionsEventProcessor>(model);
    ...
    co_return co_await providers::execute_sse_stream(
            providers::SseStreamExecutionOptions{
