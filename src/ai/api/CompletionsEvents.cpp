...
[[nodiscard]] CompletionsProviderError provider_error(const JsonObject& event) {
    ...
    return CompletionsProviderError{
            ...
            .suggested_backoff_ms = providers::provider_backoff_hint_ms(
                    event,
...
void apply_usage(
    ...
    CompletionsUsageFields fields{
            ...
            .cached_tokens = input_details
                    ? json_integer_member(*input_details, "cached_tokens")
                    : json_integer_member(usage, "cached_tokens"),
            .prompt_cache_hit_tokens =
                    json_integer_member(usage, "prompt_cache_hit_tokens"),
            .cache_write_tokens = input_details
                    ? json_integer_member(*input_details, "cache_write_tokens")
                    : std::nullopt,
            .reasoning_tokens = [&usage]() -> std::optional<std::int64_t> {
                const auto* details = json_object_member(
                        usage,
                        "completion_tokens_details");
                return details
                        ? json_integer_member(*details, "reasoning_tokens")
                        : std::nullopt;
            }(),
    };
...
[[nodiscard]] support::ExpectedVoid append_reasoning_detail(
        const JsonObject& detail,
    ...
    auto& thinking = std::get<ThinkingContent>(
            assistant.content[*thinking_index]);
...
[[nodiscard]] support::ExpectedVoid emit_text_delta(
...
[[nodiscard]] support::ExpectedVoid emit_thinking_delta(
        const std::string& delta,
...
[[nodiscard]] support::Expected<ToolSlot*> find_or_create_tool(
        const JsonObject& tool_call,
...
[[nodiscard]] support::ExpectedVoid process_tool_calls(
        const JsonObject& delta,
        std::map<std::size_t, ToolSlot>& slots,
        std::map<std::string, std::size_t, std::less<>>& ids,
        std::size_t& next_key,
        AssistantMessage& assistant,
    ...
    for (const auto& value : *calls) {
        ...
        if (!tool_call) {
            return std::unexpected(providers::make_stream_error(
                    "Malformed OpenAI Chat Completions tool call",
                    "tool_calls entry must be an object"));
        }
        auto slot = find_or_create_tool(
                *tool_call,
                slots,
                ids,
                next_key,
        ...
        auto& call = std::get<ToolCallContent>(
                assistant.content[(*slot)->content_index]);
        const auto* function = json_object_member(*tool_call, "function");
        if (!function) {
            continue;
        }
        if (const auto name = json_string_member(*function, "name");
                name && call.name.empty()) {
            call.name = std::string{*name};
        }
        const auto arguments = json_string_member(*function, "arguments");
        const std::string delta_arguments = arguments
...
[[nodiscard]] support::ExpectedVoid finish_blocks(
    ...
    for (std::size_t content_index = 0;
            content_index < assistant.content.size();
            ++content_index) {
        ...
        const auto found = std::ranges::find_if(
                slots,
                [content_index](const auto& entry) {
                    return entry.second.content_index == content_index;
                });
        if (found == slots.end()) {
            continue;
        }
...
[[nodiscard]] support::ExpectedVoid set_finish_reason(
    ...
    assistant.error_message =
            "Provider finish_reason: " + std::string{finish_reason};
    return {};
}
...
struct CompletionsEventProcessor::Impl {
    explicit Impl(Model configured_model)
...
CompletionsEventProcessor::CompletionsEventProcessor(Model model)
    : impl_(std::make_unique<Impl>(std::move(model))) {}

CompletionsEventProcessor::CompletionsEventProcessor(
        CompletionsEventProcessor&&) noexcept = default;
CompletionsEventProcessor& CompletionsEventProcessor::operator=(
        CompletionsEventProcessor&&) noexcept = default;
...
support::Expected<CompletionsProcessOutcome> CompletionsEventProcessor::process(
    ...
    if (const auto id = json_string_member(event, "id");
            id && !id->empty() && !assistant.response_id) {
        assistant.response_id = std::string{*id};
    }
    if (const auto model = json_string_member(event, "model");
            model && !model->empty() && *model != impl_->model.id &&
    ...
    if (!choice) {
        return std::unexpected(providers::make_stream_error(
                "Malformed OpenAI Chat Completions chunk",
    ...
    if (delta) {
        if (const auto content = json_string_member(*delta, "content");
                content) {
            auto emitted = emit_text_delta(
                    std::string{*content},
                    assistant,
                    impl_->text_index,
                    sink);
        ...
        for (const auto field : {"reasoning_content", "reasoning", "reasoning_text"}) {
            if (const auto reasoning = json_string_member(*delta, field);
                    reasoning && !reasoning->empty()) {
                const std::string signature =
                        impl_->model.provider == "opencode-go" &&
                                std::string_view{field} == "reasoning"
                        ? "reasoning_content"
                        : field;
                auto emitted = emit_thinking_delta(
                        std::string{*reasoning},
                        signature,
                        assistant,
                        impl_->thinking_index,
                        sink);
                if (!emitted) {
                    return std::unexpected(emitted.error());
                }
        ...
        auto processed_tools = process_tool_calls(
                *delta,
                impl_->tool_slots,
                impl_->tool_ids,
                impl_->next_tool_key,
    ...
    if (finish) {
        auto finish_set = set_finish_reason(
                *finish,
                assistant,
                impl_->saw_finish_reason);
        if (!finish_set) {
            return std::unexpected(finish_set.error());
        }
        if (!impl_->blocks_finished) {
            auto finished = finish_blocks(
                    impl_->tool_slots,
                    assistant,
                    sink);
            if (!finished) {
...
support::ExpectedVoid CompletionsEventProcessor::finish(
        AssistantMessage& assistant) {
    if (impl_->thinking_index && !impl_->reasoning_details.empty()) {
        auto& thinking = std::get<ThinkingContent>(
                assistant.content[*impl_->thinking_index]);
        auto serialized = support::write_json(
                support::JsonValue{impl_->reasoning_details});
        if (serialized) {
            thinking.thinking_signature = std::move(*serialized);
        }
    }
    ...
    if (assistant.stop_reason == AssistantStopReason::Error) {
        return std::unexpected(providers::make_stream_error(
                assistant.error_message.value_or(
