...
[[nodiscard]] ToolRenderedText render_write_call(const ToolRenderContext& context) {
    ...
    if (!content.has_value()) {
        // pi `write.ts:105`: the invalid-content notice, behind a blank line.
        rendered.blocks.push_back(
                "\n\n" + context.theme.foreground(ThemeToken::Error, "[invalid content arg - expected string]"));
        return rendered;
    }
    // pi `write.ts:106`: an empty content string is the header and nothing
    // else, with no body and no blank line.
    if (content->empty()) return rendered;

    // pi `write.ts:113`: `maxLines` is the whole file when expanded, the first
    ...
    // pi `write.ts:116`: plain text, no highlighting, tabs as three spaces.
    std::string body = "\n\n";
    bool first = true;
    ...
    // pi `write.ts:118`: write is the one renderer whose hint carries the
...
[[nodiscard]] ToolRenderedText render_write_result(const ToolRenderedResult& result, const ToolRenderContext& context) {
    // pi `write.ts:130`: a successful write renders nothing at all, because
    ...
    // pi `write.ts:136`: an error with no text renders nothing either.
