...
[[nodiscard]] inline std::string get_provider_login_help(
    ...
    const std::string_view auth_methods =
        provider == "kimi-coding" ? "API key" : "OAuth or API key";
    return "Use /login to log into a provider via " + std::string{auth_methods} + ". See:\n"
           "  " +
