...
[[nodiscard]] util::Error invalid_terminal_text(std::string message, std::string detail = {}) {
...
[[nodiscard]] std::string normalized_text(const std::vector<TerminalToken>& tokens) {
    std::string result;
    for (const auto& token : tokens) result += token.text;
    return result;
}

[[nodiscard]] util::Expected<std::size_t> token_width(const std::vector<TerminalToken>& tokens) {
    std::size_t width = 0;
...
util::Expected<std::vector<std::string>> wrap_text(std::string_view text, std::size_t width) {
    if (width == 0) {
        return std::unexpected(invalid_terminal_text("Wrap width must be positive"));
    }
    auto tokens = tokenize_terminal_output(text);
    if (!tokens) return std::unexpected(tokens.error());

    const auto is_whitespace = [](const TerminalToken& token) {
        if (token.kind != TerminalTokenKind::Grapheme) return false;
        const auto [codepoint, bytes] = decode_utf8(token.text, 0);
    ...
    std::vector<TerminalToken> pending_separator;
    ...
    const auto append_token = [&](const TerminalToken& token) {
        line += token.text;
    ...
    const auto replay_pending = [&](bool keep_whitespace) {
        for (const auto& pending : pending_separator) {
            if (keep_whitespace || pending.kind != TerminalTokenKind::Grapheme) {
    ...
    while (index < tokens->size()) {
        ...
        while (word_end < tokens->size()) {
            ...
            if (word_token.kind == TerminalTokenKind::Newline || is_whitespace(word_token)) break;
            if (first_grapheme_width == 0 && word_token.kind == TerminalTokenKind::Grapheme) {
        ...
        while (index < word_end) {
            ...
            if (word_token.kind != TerminalTokenKind::Grapheme) {
...
util::Expected<std::string> truncate_text(
    ...
    auto tokens = tokenize_terminal_output(text);
    if (!tokens) return std::unexpected(tokens.error());
    auto width_result = token_width(*tokens);
    ...
    auto ellipsis_tokens = tokenize_terminal_output(ellipsis);
    if (!ellipsis_tokens) return std::unexpected(ellipsis_tokens.error());
    for (const auto& token : *ellipsis_tokens) {
        if (token.kind != TerminalTokenKind::Grapheme) {
            return std::unexpected(invalid_terminal_text("Truncation ellipsis must contain only visible text"));
        }
    }
