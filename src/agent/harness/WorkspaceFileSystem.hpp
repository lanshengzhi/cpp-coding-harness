...
private:
    ...
    /// parent+filename walk cannot open them. Reports the underlying errno
    /// through `failure_errno` when non-null.
    [[nodiscard]] support::Expected<support::UniqueFd> open_root_directory(const std::filesystem::path& root,
            std::string_view description = "root directory",
            int* failure_errno = nullptr) const;
