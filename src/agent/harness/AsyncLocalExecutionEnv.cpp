            ...
            [runtime_target = std::move(runtime_target),
                ...
                try {
                    ...
                    admission = lane == AdmissionLane::Reserved
                                        ? runtime_target->try_admit_reserved(filesystem_detail::saturating_add(
                                                  byte_charge, filesystem_detail::kAdmittedOperationOverheadBytes))
                                        : runtime_target->try_admit(filesystem_detail::saturating_add(
                                                  byte_charge, filesystem_detail::kAdmittedOperationOverheadBytes));
                    if (!admission) {
                        completion(std::unexpected(busy_file_error(std::move(path))));
                        return;
...
struct AsyncLocalExecutionEnv::Impl final {
    Impl(std::shared_ptr<RuntimeTarget> runtime_target, std::shared_ptr<SyncLocalExecutionEnv> sync)
...
AsyncLocalExecutionEnv::AsyncLocalExecutionEnv(std::shared_ptr<RuntimeTarget> runtime_target,
        std::filesystem::path workspace,
        bool bash_enabled,
        std::vector<std::string> secret_environment_names,
        ShellConfig shell_config)
    : impl_(std::make_shared<Impl>(std::move(runtime_target),
              std::make_shared<SyncLocalExecutionEnv>(std::move(workspace),
                      bash_enabled,
...
const std::filesystem::path& AsyncLocalExecutionEnv::workspace() const { return impl_->sync->workspace(); }

support::AsyncResult<std::string, FileError> AsyncLocalExecutionEnv::absolutePath(
        std::string path, std::stop_token stop_token) {
    return submit_filesystem_operation<std::string>(impl_->runtime_target,
            impl_->sync,
            path.size(),
            stop_token,
            path,
            [path = std::move(path)](const SyncLocalExecutionEnv& sync) { return sync.absolutePath(path); });
}
...
support::AsyncResult<std::string, FileError> AsyncLocalExecutionEnv::joinPath(
    ...
    return submit_filesystem_operation<std::string>(impl_->runtime_target,
            ...
            stop_token,
            std::nullopt,
            [parts = std::move(parts)](const SyncLocalExecutionEnv& sync) { return sync.joinPath(parts); });
}
...
support::AsyncResult<std::string, FileError> AsyncLocalExecutionEnv::readTextFile(
    ...
    return submit_filesystem_operation<std::string>(impl_->runtime_target,
            ...
            [path = std::move(path), stop_token](
                    const SyncLocalExecutionEnv& sync) { return sync.readTextFile(path, stop_token); });
}
...
support::AsyncResult<std::vector<std::string>, FileError> AsyncLocalExecutionEnv::readTextLines(
    ...
    return submit_filesystem_operation<std::vector<std::string>>(impl_->runtime_target,
            impl_->sync,
            filesystem_detail::text_lines_charge(path.size()),
            stop_token,
            path,
            [path = std::move(path), maxLines, stop_token](
                    const SyncLocalExecutionEnv& sync) { return sync.readTextLines(path, maxLines, stop_token); });
}

support::AsyncResult<BinaryData, FileError> AsyncLocalExecutionEnv::readBinaryFile(
        std::string path, std::stop_token stop_token) {
    return submit_filesystem_operation<BinaryData>(impl_->runtime_target,
            impl_->sync,
            filesystem_detail::file_read_charge(path.size()),
            stop_token,
            path,
            [path = std::move(path), stop_token](
                    const SyncLocalExecutionEnv& sync) { return sync.readBinaryFile(path, stop_token); });
}
...
support::AsyncResult<void, FileError> AsyncLocalExecutionEnv::writeFile(
    ...
    const auto byte_charge = filesystem_detail::saturating_add(
            path.size(), std::visit([](const auto& value) { return value.size(); }, content));
    return submit_filesystem_operation<void>(impl_->runtime_target,
            impl_->sync,
            byte_charge,
            stop_token,
            path,
            [path = std::move(path), content = std::move(content), stop_token](
                    const SyncLocalExecutionEnv& sync) { return sync.writeFile(path, content, stop_token); });
}
...
support::AsyncResult<void, FileError> AsyncLocalExecutionEnv::appendFile(
    ...
    return submit_filesystem_operation<void>(impl_->runtime_target,
            ...
            path,
            [path = std::move(path), content = std::move(content), stop_token](
                    const SyncLocalExecutionEnv& sync) { return sync.appendFile(path, content, stop_token); });
}

support::AsyncResult<FileInfo, FileError> AsyncLocalExecutionEnv::fileInfo(
        std::string path, std::stop_token stop_token) {
    return submit_filesystem_operation<FileInfo>(impl_->runtime_target,
            impl_->sync,
            path.size(),
            stop_token,
            path,
            [path = std::move(path)](const SyncLocalExecutionEnv& sync) { return sync.fileInfo(path); });
}

support::AsyncResult<std::vector<FileInfo>, FileError> AsyncLocalExecutionEnv::listDir(
        std::string path, std::stop_token stop_token) {
    return submit_filesystem_operation<std::vector<FileInfo>>(impl_->runtime_target,
            impl_->sync,
            filesystem_detail::directory_list_charge(path.size()),
            stop_token,
            path,
            [path = std::move(path), stop_token](
                    const SyncLocalExecutionEnv& sync) { return sync.listDir(path, stop_token); });
}
...
support::AsyncResult<std::string, FileError> AsyncLocalExecutionEnv::canonicalPath(
    ...
    return submit_filesystem_operation<std::string>(impl_->runtime_target,
            ...
            stop_token,
            path,
            [path = std::move(path)](const SyncLocalExecutionEnv& sync) { return sync.canonicalPath(path); });
}

support::AsyncResult<bool, FileError> AsyncLocalExecutionEnv::exists(std::string path, std::stop_token stop_token) {
    return submit_filesystem_operation<bool>(impl_->runtime_target,
            impl_->sync,
            path.size(),
            stop_token,
            path,
            [path = std::move(path)](const SyncLocalExecutionEnv& sync) { return sync.exists(path); });
}
...
support::AsyncResult<void, FileError> AsyncLocalExecutionEnv::createDir(
    ...
    return submit_filesystem_operation<void>(impl_->runtime_target,
            ...
            [path = std::move(path), recursive](
                    const SyncLocalExecutionEnv& sync) { return sync.createDir(path, recursive); });
}
...
support::AsyncResult<void, FileError> AsyncLocalExecutionEnv::remove(
    ...
    return submit_filesystem_operation<void>(impl_->runtime_target,
            ...
            [path = std::move(path), recursive, stop_token](
                    const SyncLocalExecutionEnv& sync) { return sync.remove(path, recursive, stop_token); });
}
...
support::AsyncResult<std::string, FileError> AsyncLocalExecutionEnv::createTempDir(
    ...
    return submit_filesystem_operation<std::string>(impl_->runtime_target,
            ...
            stop_token,
            std::nullopt,
            [prefix = std::move(prefix)](const SyncLocalExecutionEnv& sync) { return sync.createTempDir(prefix); });
}
...
support::AsyncResult<std::string, FileError> AsyncLocalExecutionEnv::createTempFile(
    ...
    const auto byte_charge =
            filesystem_detail::saturating_add(prefix ? prefix->size() : 0, suffix ? suffix->size() : 0);
    return submit_filesystem_operation<std::string>(impl_->runtime_target,
            impl_->sync,
            byte_charge,
            stop_token,
            ...
            [prefix = std::move(prefix), suffix = std::move(suffix)](
                    const SyncLocalExecutionEnv& sync) { return sync.createTempFile(prefix, suffix); });
}
...
support::AsyncResult<ShellExecResult, ExecutionError> AsyncLocalExecutionEnv::exec(
    ...
    auto impl = impl_;
    return support::AsyncResult<ShellExecResult,
            ExecutionError>{support::AsyncProducer<ShellExecResult, ExecutionError>{
            [impl = std::move(impl), command = std::move(command), options = std::move(options), byte_charge](
                ...
                try {
                    ...
                    state->install(std::move(*admission), std::move(completion), impl->runtime_target, impl->sync);
                    const bool queued = state->admission->post_worker(
                            [state, command = std::move(command), options = std::move(options)]() mutable noexcept {
#if !defined(BOOST_ASIO_NO_EXCEPTIONS)
                                try {
#endif
                                    state->request =
                                            state->sync->make_exec_request(std::move(command), std::move(options));
#if !defined(BOOST_ASIO_NO_EXCEPTIONS)
                                } catch (...) {
                                    state->complete(std::unexpected(unknown_execution_error()));
                                    return;
                                ...
                                if (!state->admission->post_loop([state]() noexcept {
                                        ...
                                        try {
                                            ...
                                            if (!state->request || !*state->request) {
                                                state->complete(
                                                        std::unexpected(state->request ? state->request->error()
