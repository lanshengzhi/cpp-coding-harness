...
namespace cch::harness::session::detail {

// pi v3 Session Format message DTOs and their mapping to `cch::ai::Message`
// values. The shapes are deliberately duplicated from the AI message model:
// the pi wire contract, not structural identity with `cch::ai::MessageVariant`,
// is what is pinned (ADR 0056).
//
// The DTOs must have external linkage: Glaze reflects them through
// glz::detail::external, which Clang rejects for anonymous-namespace types
// (issue #487; the Clang conformance build).
//
// Glaze writes members in declaration order and reflects member names onto
// wire field names: keep this declaration order, field-name spelling, and
// optionality — the session file is byte-stable under the golden tests.
...
[[nodiscard]] inline support::Expected<std::vector<ai::Content>> required_content_from_dto(
        const std::optional<MessageContentDto>& content, std::string_view role, std::string_view context) {
    if (!content) {
        return std::unexpected(json_contract_error("missing required JSON field",
    ...
    if (std::holds_alternative<std::string>(*content)) {
        return std::unexpected(json_contract_error("content must be a block array",
                std::string{"content for "} + std::string(role) + " must be a JSON array, found a JSON string",
                context));
    }
...
[[nodiscard]] inline support::Expected<std::variant<std::string, std::vector<ai::Content>>> user_content_from_dto(
        const std::optional<MessageContentDto>& content, std::string_view context) {
    if (!content) {
        return std::unexpected(json_contract_error(
                "missing required JSON field", "missing required field 'content' for user message", context));
    }
    if (const auto* text = std::get_if<std::string>(&*content)) {
        return std::variant<std::string, std::vector<ai::Content>>{*text};
    }
    auto blocks = content_from_dto(std::get<std::vector<ContentDto>>(*content), context);
    if (!blocks) {
        return std::unexpected(blocks.error());
    }
    return std::variant<std::string, std::vector<ai::Content>>{std::move(*blocks)};
}

[[nodiscard]] inline UsageDto to_dto(const ai::Usage& usage) {
    return UsageDto{
            .input = usage.input,
            .output = usage.output,
...
[[nodiscard]] inline support::Expected<ai::Usage> usage_from_dto(const UsageDto& dto, std::string_view context) {
    ...
    if (auto required = require_field(cost.cacheRead, "assistant usage cost", "usage.cost.cacheRead", context);
            !required) {
        return std::unexpected(required.error());
    }
    if (auto required = require_field(cost.cacheWrite, "assistant usage cost", "usage.cost.cacheWrite", context);
            !required) {
        return std::unexpected(required.error());
    }
...
[[nodiscard]] inline ContentDto to_dto(const ai::TextContent& content) {
    return ContentDto{
            .type = "text",
            .text = content.text,
...
[[nodiscard]] inline ContentDto to_dto(const ai::ThinkingContent& content) {
    return ContentDto{
            .type = "thinking",
...
[[nodiscard]] inline ContentDto to_dto(const ai::ImageContent& content) {
    return ContentDto{
            .type = "image",
            .data = content.data,
...
[[nodiscard]] inline ContentDto to_dto(const ai::ToolCallContent& content) {
    return ContentDto{
            .type = "toolCall",
            .id = content.id,
            .name = content.name,
            .arguments = content.arguments ? std::optional<glz::generic>{support::json_to_glaze(*content.arguments)}
                                           : std::nullopt,
...
[[nodiscard]] inline ContentDto to_dto(const ai::Content& content) {
    return std::visit([](const auto& concrete) { return to_dto(concrete); }, content);
}

[[nodiscard]] inline ContentDto to_dto(const ai::AssistantContent& content) {
    return std::visit([](const auto& concrete) { return to_dto(concrete); }, content);
}
...
[[nodiscard]] inline support::Expected<ai::Content> content_from_dto(const ContentDto& dto, std::string_view context) {
    ...
    if (dto.type == "image") {
        ...
        return ai::Content{ai::ImageContent{
                .data = *dto.data,
                .mime_type = *dto.mimeType,
        }};
    }
    if (dto.type == "toolCall") {
        return std::unexpected(json_contract_error("unexpected toolCall in non-assistant content",
                "toolCall content blocks are only valid in assistant messages",
                context));
    }

    return std::unexpected(
            json_contract_error("unknown content discriminator", "unknown content type '" + dto.type + "'", context));
}
...
[[nodiscard]] inline support::Expected<ai::AssistantContent> assistant_content_from_dto(
    ...
    if (dto.type == "toolCall") {
        ...
        return ai::AssistantContent{ai::ToolCallContent{
                .id = *dto.id,
                .name = *dto.name,
                .arguments = dto.arguments ? std::optional<support::JsonValue>{support::json_from_glaze(*dto.arguments)}
                                           : std::nullopt,
    ...
    if (dto.type == "image") {
        return std::unexpected(json_contract_error("unexpected image in assistant content",
...
[[nodiscard]] inline support::Expected<std::vector<ai::AssistantContent>> required_assistant_content_from_dto(
    ...
    if (!content) {
        return std::unexpected(json_contract_error(
                "missing content", std::string(role) + " message is missing the 'content' field", context));
    }
    if (std::holds_alternative<std::string>(*content)) {
        return std::unexpected(json_contract_error("content must be a block array",
                std::string(role) + " message content must be a JSON array, found a JSON string",
                context));
    }
    return assistant_content_from_dto(std::get<std::vector<ContentDto>>(*content), context);
}

[[nodiscard]] inline std::vector<ContentDto> to_content_dtos(const std::vector<ai::Content>& content) {
...
[[nodiscard]] inline DiagnosticErrorInfoDto to_dto(const ai::DiagnosticErrorInfo& info) {
...
[[nodiscard]] inline ai::DiagnosticErrorInfo diagnostic_error_info_from_dto(const DiagnosticErrorInfoDto& dto) {
    ai::DiagnosticErrorInfo info;
    info.name = dto.name;
...
[[nodiscard]] inline DiagnosticEntryDto to_dto(const ai::DiagnosticEntry& entry) {
...
[[nodiscard]] inline ai::DiagnosticEntry diagnostic_entry_from_dto(const DiagnosticEntryDto& dto) {
    ai::DiagnosticEntry entry;
    entry.type = dto.type;
...
[[nodiscard]] inline std::vector<DiagnosticEntryDto> to_diagnostic_entry_dtos(
        const std::vector<ai::DiagnosticEntry>& entries) {
...
[[nodiscard]] inline support::Expected<std::vector<ai::DiagnosticEntry>> diagnostic_entries_from_dto(
        const std::vector<DiagnosticEntryDto>& dtos, std::string_view /*context*/) {
    std::vector<ai::DiagnosticEntry> entries;
...
[[nodiscard]] inline support::Expected<std::vector<ai::DiagnosticEntry>> required_diagnostic_entries_from_dto(
        const std::optional<std::vector<DiagnosticEntryDto>>& dtos, std::string_view context) {
    if (!dtos) {
        return std::vector<ai::DiagnosticEntry>{};
    }
...
[[nodiscard]] inline std::optional<MessageContentDto> to_dto_content(const ai::UserMessage& message) {
    ...
    return MessageContentDto{to_content_dtos(std::get<std::vector<ai::Content>>(message.content))};
}

[[nodiscard]] inline MessageDto to_message_dto(const ai::UserMessage& message) {
    return MessageDto{
            .role = "user",
...
[[nodiscard]] inline MessageDto to_message_dto(const ai::AssistantMessage& message) {
    return MessageDto{
            ...
            .diagnostics = message.diagnostics
                                   ? std::optional<std::vector<DiagnosticEntryDto>>{to_diagnostic_entry_dtos(
                                             *message.diagnostics)}
                                   : std::nullopt,
            .timestamp = message.timestamp,
    };
...
[[nodiscard]] inline MessageDto to_message_dto(const ai::BashExecutionMessage& message) {
    return MessageDto{
            .role = "bashExecution",
            .command = message.command,
            .output = message.output,
            .exitCode = message.exit_code ? std::optional<std::int64_t>{static_cast<std::int64_t>(*message.exit_code)}
                                          : std::nullopt,
...
[[nodiscard]] inline MessageDto to_message_dto(const ai::CustomMessage& message) {
    return MessageDto{
            .role = "custom",
            .content = to_content_dtos(message.content),
            .details = message.details ? std::optional<glz::generic>{support::json_to_glaze(*message.details)}
...
[[nodiscard]] inline MessageDto to_message_dto(const ai::BranchSummaryMessage& message) {
    return MessageDto{
            .role = "branchSummary",
...
[[nodiscard]] inline MessageDto to_message_dto(const ai::CompactionSummaryMessage& message) {
    return MessageDto{
...
[[nodiscard]] inline support::Expected<ai::MessageVariant> message_from_dto(
    ...
    if (dto.role == "user") {
        ...
        return ai::MessageVariant{ai::UserMessage{
    ...
    if (dto.role == "assistant") {
        ...
        std::optional<std::vector<ai::DiagnosticEntry>> diagnostics;
        ...
        return ai::MessageVariant{ai::AssistantMessage{
                .content = std::move(*content),
                .api = *dto.api,
    ...
    if (dto.role == "toolResult") {
        ...
        return ai::MessageVariant{ai::ToolResultMessage{
    ...
    if (dto.role == "bashExecution") {
        ...
        return ai::MessageVariant{ai::BashExecutionMessage{
                .command = *dto.command,
                .output = dto.output.value_or(""),
                .exit_code = dto.exitCode ? std::optional<int>{static_cast<int>(*dto.exitCode)} : std::nullopt,
    ...
    if (dto.role == "custom") {
        ...
        return ai::MessageVariant{ai::CustomMessage{
    ...
    if (dto.role == "branchSummary") {
        ...
        return ai::MessageVariant{ai::BranchSummaryMessage{
                .summary = *dto.summary,
                .from_id = *dto.fromId,
                .timestamp = dto.timestamp,
        }};
    ...
    if (dto.role == "compactionSummary") {
        ...
        return ai::MessageVariant{ai::CompactionSummaryMessage{
                .summary = *dto.summary,
    ...
    return std::unexpected(
            json_contract_error("unknown message role", "unknown message role '" + dto.role + "'", context));
}
