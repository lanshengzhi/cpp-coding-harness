...
- [x] CLI and SDK session creation default to the user-level trust store.
- [x] The SDK accepts an absolute trust-store path beneath a canonical external parent, including a not-yet-created trust file.
- [x] Relative paths, the workspace itself, workspace descendants, canonicalization errors, and indeterminate containment fail closed.
- [x] Existing files, symlinked files, and symlinked parent directories cannot bypass workspace containment checks.
- [x] Workspace-local trust state cannot authorize discovered project resources.
- [x] The public SDK contract documents the additive trust-store option and its containment rules.
