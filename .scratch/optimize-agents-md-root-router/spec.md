# Spec: Optimize AGENTS.md as a Root Router
...
14. As an agent working on pi parity, I want the pi C++ direction and its planning authority discoverable, so that parity work follows current decisions.
...
27. As an agent following local issue-tracker conventions, I want the spec and any derived implementation issues to use the ready-for-agent status, so that the work can be picked up without extra triage.
...
- Existing pi C++ parity direction should stay visible, with unresolved decisions left in the dedicated wayfinder map.
- Do not change C++ contracts, CLI behavior, session schema, provider behavior, tool behavior, or build configuration as part of this spec.
...
- Validate that pi C++ parity guidance points agents toward the dedicated wayfinder map.
...
- Prior art: the existing completed cleanup plan for progressive disclosure already established that build commands and stale implementation details should be referenced rather than duplicated. This spec extends that direction by splitting detailed routing reference out of the root instructions.
...
- Creating implementation issues beyond this spec unless the maintainer asks for task breakdown.
- Modifying local issue-tracker conventions or triage vocabulary.

## Further Notes

- No ADR files were found for this area during exploration.
- The domain glossary reserves “Seam” for module/interface boundaries; this spec uses “RootRouter” for `AGENTS.md`.
- The current working tree was clean before publishing this spec.
- This spec intentionally synthesizes from the current conversation and repo state without interviewing the user further.

## Post-build Review

- 2026-07-03: Reviewed as an already-applied documentation refactor. `Status: implemented` is the completed local lifecycle state; acceptance checkboxes and review comments carry the completion evidence.
