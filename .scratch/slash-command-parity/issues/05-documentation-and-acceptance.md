...
**What to build:** Make user-facing documentation and the parity planning authority accurately describe the implemented Phase 1 command set, then run the complete focused acceptance slice.
...
- [x] Record the current slash-command behavior without claiming direct pi parity for CLI adaptations; current behavior now lives in `README.md` and parity decisions live in `.scratch/pi-cpp-parity/map.md`.
- [x] Check spec acceptance criteria against the final implementation and record any intentionally deferred item rather than silently broadening scope.
...
All spec acceptance criteria were checked and satisfied. Direct pi slash-command parity beyond this Phase 1 line-CLI slice remains intentionally deferred, including operational `/new` and `/resume`, `/compact`, and additional pi built-ins; future scope decisions belong in the parity wayfinder map. `/help`, `/commands`, `/clear`, and `/exit` remain explicitly classified as C++ line-CLI adaptations rather than direct pi parity.

Code review found two documentation consistency issues, both corrected before handoff: the then-current parity inventory pointed to the old registry location and four-command baseline, and the implemented spec described its pre-implementation baseline in present tense. Standards review found no remaining baseline code smells; spec review found no scope creep.
