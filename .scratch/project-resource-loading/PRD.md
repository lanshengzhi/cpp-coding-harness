# Deepen Project Resource Loading
...
- Preserve the current coarse resource policy for this slice. The existing project skills enablement continues to gate project skills and project prompt templates unless a later PRD introduces per-resource toggles.
...
- Future resource markers such as project settings, packages, extensions, and project system prompt files remain unsupported in this PRD. They should flow through the plan as unsupported decisions, not through ad hoc checks.
...
- This PRD follows `.scratch/architecture-review-todo.md` candidate 2: `resource-loading`.
...
- Relevant local plans include the pi parity roadmap and the completed skill loading, skill model-visible integration, project trust/resource controls, and prompt template loading plans.
