# Spec: Align AgentSession events, prompting, and persistence with pi
...
47. As a maintainer, I want parity documentation synchronized, so that future work starts from the implemented contract rather than stale C++ adaptations.
...
- Update the domain glossary, module routing, README, and parity planning authority to describe actual ownership and supported behavior. Remove claims about C++ JSON schema v1, runtime-terminal records, per-prompt sinks, committed-only state, or SDK commands.
...
- The spec is intentionally source-breaking. The repository describes its SDK as experimental and not compatibility-preserving, so old interfaces must be deleted rather than carried forward.
