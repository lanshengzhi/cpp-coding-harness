...
- R3. Build targets reflect pi package dependency direction: `coding_agent/runtime` depends on `agent` and `harness`; `agent` depends on `ai` and `harness` contracts; provider implementations stay under `ai`.
...
- **Split CMake targets by package boundary.** Create `cch_ai`, `cch_agent`, `cch_harness`, and `cch_coding_agent_runtime` library targets (exact naming to be decided during implementation) instead of one `cpp_harness_lib`. This makes dependency violations a build failure and mirrors pi's package graph.
...
graph TB
    ...
    agent["agent<br/>(loop, state, events, tools)"]
...
- Introduce library targets aligned with the package map (e.g., `cch_util`, `cch_ai`, `cch_agent`, `cch_harness`, `cch_coding_agent_runtime`).
...
- Add architecture tests that inspect `CMakeLists.txt`, target link dependencies, and source/include relationships to verify:
  - `agent` does not link `coding_agent_runtime`.
  - `ai` does not link `agent` or `coding_agent_runtime`.
