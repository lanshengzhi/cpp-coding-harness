...
| `pi:packages/agent` | `include/cch/agent/`, `src/agent/`, `include/cch/harness/`, `src/harness/`, `tests/agent/`, `tests/harness/` | Partial: async loop/state, pi-shaped supported lifecycle events, move-only sinks, tool hooks and scheduling, execution capabilities, incremental JSONL persistence, and session-tree reconstruction exist. Public queue control, progress events, retry, and compaction execution remain deferred. | `pi:packages/agent/src/types.ts`, `pi:packages/agent/src/agent-loop.ts`, `pi:packages/agent/src/harness/types.ts`, `pi:packages/agent/src/harness/agent-harness.ts` |
| `pi:packages/coding-agent` core/CLI | `src/cli/`, `src/coding_agent/runtime/`, `src/main.cpp`, `include/cch/coding_agent/Sdk.hpp`, `tests/cli/`, `tests/coding_agent/` | Partial: one-shot/REPL/resume, centralized session assembly, CLI-owned built-ins, skill/template-only AgentSession prompting, live-state accessors, persistent subscriptions, direct JSON events, narrow RPC, and SDK v1 exist. Session replacement, full RPC command parity, extensions, compaction/retry orchestration, and TUI remain deferred. | `pi:packages/coding-agent/src/core/`, `pi:packages/coding-agent/src/cli/`, `pi:packages/coding-agent/docs/usage.md`, `pi:packages/coding-agent/docs/json.md`, `pi:packages/coding-agent/docs/rpc.md` |
...
- [x] `src/cli/`, `src/main.cpp`, and `src/coding_agent/runtime/` split current runtime wiring into CLI parsing, session assembly, runtime services, session lifecycle, event printing, and agent execution seams.
...
- [x] Classify pi features into MVP parity, near-term parity, and intentionally deferred parity.
  - **Done when:** README and this TODO agree on which gaps are intentional: full TUI, extensions, package installation, OAuth, broader provider/model catalogs, session replacement and compaction execution, full RPC/SDK parity, and sandbox/container integration.
...
- [x] Split CLI/runtime wiring into pi-coding-agent-like responsibilities.
  - **Files:** `src/coding_agent/runtime/AsyncCliRuntime.*`, `src/main.cpp`, `src/coding_agent/runtime/`, `tests/cli/CliSmokeTest.cpp`.
...
- [x] Add slash-command and prompt-processing seams after runtime/session boundaries are stable.
  - **Files:** `src/coding_agent/prompt/PromptProcessor.*`, `src/coding_agent/prompt/PromptTemplateExpander.*`, `src/coding_agent/runtime/AgentSessionRuntime.*`, `tests/coding_agent/PromptProcessorTest.cpp`, `tests/coding_agent/PromptTemplateProcessingTest.cpp`, `tests/coding_agent/BuiltinCommandsTest.cpp`.
  ...
  - **Status:** The private owning `PromptProcessor` performs cached skill expansion followed by prompt-template expansion for every AgentSession prompt; unmatched slash input passes through silently, and `PromptOptions::expand_prompt_templates` can bypass both stages. CLI built-in commands are dispatched before AgentSession, while RPC JSON commands remain owned by `RpcMode`. `CommandRegistry` has no AgentSession or SDK ownership. Legacy public prompt-processing free functions have been removed; template substitution covers pi's positional, default, slice, quote, whitespace, and single-pass semantics.
- [ ] Implement the remaining pi-aligned built-in slash commands.
  ...
  - [x] **Phase 1 line-CLI slice:** the CLI adapter now resolves 8 effective names before input reaches the session: canonical `/help`, `/session`, `/quit`, `/clear`, `/new`, and `/resume`, plus `/commands` and `/exit` aliases. The built-in registry and dispatch logic live in `src/coding_agent/runtime/AsyncCliRuntime.*` and share slash parsing with `PromptProcessor` via `src/coding_agent/prompt/SlashCommandParser.*`. Metadata, alias resolution, deterministic help, and text/JSON presentation have focused coverage. Recognized CLI commands do not enter `AgentSession` or prompt history; RPC `prompt` treats the same slash-shaped text as an ordinary prompt because RPC commands are JSON records owned by `RpcMode`.
...
- [x] Implement prompt template loading and invocation.
  ...
  - **Status:** `PromptTemplateLoader` provides YAML frontmatter parsing and directory scanning. The private `PromptTemplateExpander` implementation supports multi-digit positions, `$0`, `$@`/`$ARGUMENTS`, defaults, slices including `${@:0}`, quote-aware mixed-whitespace arguments, and non-recursive substitution. Templates are owned by the session's private prompt processor rather than `RuntimeServices`.
...
- [x] Add JSON event stream mode as the first machine-readable surface.
  - **Files:** `src/main.cpp`, `src/coding_agent/runtime/AsyncCliRuntime.*`, `src/coding_agent/runtime/JsonEventPrinter.*`, `tests/coding_agent/runtime/JsonEventPrinterTest.cpp`, `tests/cli/CliSmokeTest.cpp`.
...
- [x] Add RPC mode only after runtime services are separated from CLI printing.
  - **Files:** `src/main.cpp`, `src/coding_agent/runtime/AsyncCliRuntime.*`, `src/coding_agent/runtime/AgentSessionPromptAccess.hpp`, `src/coding_agent/runtime/RpcJsonl.*`, `src/coding_agent/runtime/RpcMode.*`, `tests/coding_agent/runtime/RpcModeTest.cpp`, `tests/cli/CliSmokeTest.cpp`.
  - **References:** `pi:packages/coding-agent/docs/rpc.md`.
  - **Implementation plan:** `docs/plans/2026-06-20-003-feat-t8-jsonl-rpc-mode-plan.md` (historical), `.scratch/pi-agent-session-event-prompt-parity/issues/14-align-rpc-preflight-and-events.md` (current alignment).
  - **Status:** `--mode rpc` reads strict JSONL commands on stdin and interleaves correlated response records with flushed, direct AgentSession events for the narrow command set `prompt`, `get_state`, `get_last_assistant_text`, and `shutdown`. Prompt success is emitted after AgentSession preflight and before events; preflight rejection is an error response; later execution outcomes create no second response. RPC emits neither a startup session header nor runtime terminal records. Full pi RPC command parity, extension UI, compaction/retry operations, and session replacement remain deferred.
