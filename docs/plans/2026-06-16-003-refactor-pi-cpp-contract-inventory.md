...
| File error and execution error code families | Partial: `include/cch/util/Error.hpp`, execution env error messages | Deferred parity | Current API preserves existing safety semantics; richer stable error code parity is later T4. |
| `FileInfo`, `FileSystem` | Partial: `include/cch/harness/ExecutionEnv.hpp` text file read/write/edit methods | Deferred parity | Full filesystem capability surface (stat/list/binary/temp dirs) is later T4. |
| `Shell`, `ExecutionEnvExecOptions`, `ExecutionEnv` | `include/cch/harness/ExecutionEnv.hpp::AsyncExecutionEnv`; `include/cch/harness/LocalExecutionEnv.hpp` | Near-term parity | U6 converts shell execution to true async I/O and preserves containment/redaction/timeout behavior. |
