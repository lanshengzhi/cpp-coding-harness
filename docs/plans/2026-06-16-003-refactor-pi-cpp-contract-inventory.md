...
| `SlashCommandInfo`, command sources | `include/cch/coding_agent/CommandRegistry.hpp` (`CommandInfo`, `CommandContext`, `CommandRegistry`) | MVP parity with C++ adaptations | Registry-owned metadata, aliases, explicit collision errors, deterministic introspection, 6 canonical built-ins, and 2 aliases are implemented. `/help`, `/commands`, `/clear`, and `/exit` are line-CLI adaptations rather than direct pi parity; extension/prompt/skill command sources remain deferred. |
| `PromptTemplate`, `expandPromptTemplate`, prompt interpretation | Public passive values and legacy wrappers in `include/cch/coding_agent/`; private owning processor in `src/coding_agent/prompt/` | MVP parity | The session processor owns commands, skills, and templates as one snapshot; dispatch order is command → cached skill → template, with silent passthrough for unmatched slash input. Template substitution covers current pi positional/default/slice and quote/whitespace semantics; `PromptOptions::expand_prompt_templates` bypasses all three stages when false. |
