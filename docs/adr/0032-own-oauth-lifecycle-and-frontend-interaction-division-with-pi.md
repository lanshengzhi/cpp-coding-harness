...
OAuth login, refresh, cancellation, expiry, persistence, logout, and user interaction are divided exactly where pi divides them at the parity baseline, as a capability subset by omission only. `cch_ai` owns the interaction content and auth orchestration for providers that still expose OAuth, the CLI/Native TUI owns presentation and cancellation, `SessionFactory` owns none of it, and SDK/RPC hosts carry no built-in login surface. This implements parity-map ticket [#328](https://github.com/lanshengzhi/cpp-coding-harness/issues/328), refining the seams frozen by [#326](https://github.com/lanshengzhi/cpp-coding-harness/issues/326) and the secret boundary frozen by [#327](https://github.com/lanshengzhi/cpp-coding-harness/issues/327). C++ remains a pi capability subset by omission only; no backward compatibility, migration shims, aliases, or fallback reads. Kimi's provider-specific exception is recorded in [ADR 0059](0059-align-the-cch-ai-capability-subset-with-pi-ai-and-diverge-on-kimi-code.md): it is API-key-only and has no OAuth lifecycle.

> SDK/RPC, public Models Runtime injection, `util::Error`, and multi-platform browser-launch clauses are superseded by [ADR 0036](0036-own-the-scoped-pi-coding-agent-application-layer-capabilities-for-the-three-provider-paths.md), [ADR 0039](0039-own-the-capability-owner-package-graph-and-parity-architecture-gate.md), and [ADR 0040](0040-own-asynchronous-operations-and-the-serialized-runtime-lifecycle.md): SDK/RPC surfaces are removed; coding-agent Models Runtime is concrete composition; shared error mechanics use `cch_support`; and browser presentation is supported only on native Linux x86-64 with glibc. The AI-owned auth interaction content, host-owned Native TUI presentation/cancellation, explicit-login rule, request-time refresh semantics, credential persistence, and logout ordering remain authoritative.
...
- `cch_ai` owns the interaction *content* and auth orchestration: the `AuthInteraction`/`AuthPrompt`/`AuthEvent` contract carried by `std::move_only_function` (guardrail 3), the provider-owned OAuth implementations that remain in scope (prompt texts, instructions, HTML pages, error messages), the shared device-polling helper, and `Models` login/logout/checkAuth/getAuth orchestration. Kimi uses the shared API-key login contract instead and has no OAuth implementation. `cch_ai` never renders.
...
### Login, persistence, refresh, and provider exceptions
...
- Kimi is deliberately excluded from this OAuth lifecycle: its vendor-authoritative path is API-key-only, and stale stored OAuth records remain inert until explicit API-key replacement or logout. See ADR 0059 and issue #763.
...
- Frozen pi baseline `83114817c68f5413e4d7ba6d7003ddc511cd31d2`: `packages/ai/src/auth/{types,resolve,helpers}.ts`, `packages/ai/src/auth/oauth/{openai-codex,device-code,pkce}.ts`, `packages/ai/src/models.ts`, `packages/coding-agent/src/core/{model-runtime,auth-storage,model-resolver}.ts`, `packages/coding-agent/src/modes/interactive/{interactive-mode.ts,components/login-dialog.ts,components/oauth-selector.ts}`, `packages/coding-agent/src/modes/rpc/*`, `packages/coding-agent/src/{main.ts,cli/args.ts,core/agent-session.ts}`. The upstream Kimi OAuth source remains provenance for the exception recorded by ADR 0059, not a C++ implementation target.
