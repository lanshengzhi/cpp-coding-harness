...
| OpenAI-compatible provider / SSE / HTTP transport | `include/cch/ai/providers/`, `src/ai/providers/`, `src/ai/glaze/`, `tests/ai/providers/` | Keep provider wire DTOs and Glaze mappings in implementation/serialization layers. Do not move provider wire details back into public domain contracts. When adding a provider or transport, preserve the `StreamingChatClient` / `StreamTransport` seams. |
...
| Built-in tools: read/write/edit/bash | `include/cch/tools/ToolFactories.hpp`, `src/tools/AsyncToolFactories.cpp`, `tests/tools/` | Tools are exposed through `AsyncAgentTool`. File and process capabilities must go through the execution environment. `edit` uses `edits[]` multi-edit semantics, `read` appends a continuation hint when truncated, and `write` implicitly creates parent directories. When adding a tool, define the `ai::Tool` schema, implement `AsyncAgentTool`, register through `AsyncToolRegistry`, and add focused tests under `tests/tools/`. |
...
| Project trust / resource controls | `include/cch/coding_agent/ProjectTrust.hpp`, `include/cch/coding_agent/ProjectResources.hpp`, `src/coding_agent/ProjectTrust.cpp`, `src/coding_agent/ProjectResources.cpp`, `src/coding_agent/ProjectResourceLoader.*`, `tests/coding_agent/Project*Test.cpp`, `tests/cli/CliSmokeTest.cpp` | Handles project-local `.cpp-harness` resource marker detection, trust store, `--approve`/`--no-approve`/`--no-skills`, load plans, and the central project resource-loading seam. Some CLI/SDK call-site migration may still live in `SessionFactory`/`RuntimeServices` until the active resource-loading plan finishes. Project trust is only a startup input-loading guard; it is not a sandbox. |
| Skills / resource loading | `include/cch/coding_agent/Skill.hpp`, `src/coding_agent/ProjectResourceLoader.*`, `src/coding_agent/SkillLoader.*`, `src/coding_agent/SkillFormatting.*`, `src/coding_agent/runtime/RuntimeServices.*`, `src/coding_agent/runtime/AgentSessionRuntime.*`, `src/coding_agent/runtime/SessionFactory.*`, `tests/coding_agent/Skill*Test.cpp` | Project-local `.cpp-harness/skills/**/SKILL.md` parsing remains delegated to `SkillLoader`; use `ProjectResourceLoader` for new shared project resource-loading policy and diagnostics work. Preserve frontmatter parsing, diagnostics, `<available_skills>` injection, and explicit `/skill:<name>` invocation. Loader/formatter implementation stays in `src/coding_agent/`. Global/config-driven skill dirs and package install remain later T6 work. |
| Workspace, path guard, and shell execution | `include/cch/harness/ExecutionEnv.hpp`, `include/cch/harness/LocalExecutionEnv.hpp`, `src/harness/` (including private `SyncLocalExecutionEnv.*`), `src/tools/PathGuard.hpp`, `src/util/Process.*`, `tests/harness/` | This seam owns path containment and safety checks. Async shell I/O must go through `ProcessRunner`. Keep private execution-env implementation details out of public include surfaces. Any shell, file, environment-variable, or session-adjacent execution change must preserve workspace containment, secret redaction, and output truncation. |
| Session JSONL / resume / tree navigation | `include/cch/harness/session/`, `src/harness/session/JsonlSessionStore.cpp`, `src/harness/session/SessionTree.cpp`, `src/harness/session/SessionResume.cpp`, `tests/harness/session/`, `tests/coding_agent/runtime/SessionLifecycleTest.cpp` | Session storage is an append-only, redacted typed transcript. Resume uses `SessionResume` to return agent-ready active-path history, effective context values, metadata, and `SessionTopology`; `SessionTree` owns tree indexing, branch navigation, and compaction-aware context reconstruction. When adding session fields, preserve append-only JSONL and tolerate unknown entries. |
