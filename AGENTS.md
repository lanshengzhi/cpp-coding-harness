...
| Config / 模型解析 | `include/cch/coding_agent/Config.hpp`, `src/coding_agent/ConfigLoader.cpp`, `src/coding_agent/ProviderConfigResolution.cpp`, `tests/coding_agent/ConfigLoaderTest.cpp`, `tests/coding_agent/ProviderConfigResolutionTest.cpp` | `~/.cpp-harness/config.json` 加载 provider/model/base-url/api-key-env、`default_project_trust` 和 project resource 默认值；`resolve_provider_settings()` 统一 CLI/config/resume 优先级；env var 链解析；session resume 保留存储的 provider/model。 |
| JSON 序列化/反序列化 | `src/ai/glaze/`, `src/util/Json.hpp`, `tests/ai/GlazeRoundTripTest.cpp` | Domain 用 `include/cch/util/JsonValue.hpp`；Glaze DTO 与 JSON adapter 留在 `src/` 实现层，只在边界做 DTO 转换。 |
...
| CLI / REPL / runtime wiring | `src/main.cpp`, `src/cli/` (`CliParse`, `CliPreflight`, `CliConfig`), `src/AsyncCliRuntime.*`, `src/coding_agent/runtime/`, `tests/cli/` | CLI11 参数解析与 preflight（含 `config.json` 优先级）在 `src/cli/`；runtime 负责组装服务并输出稳定 semantic event line；不要恢复 legacy `--async` 等兼容 flag。 |
...
| 公共边界/架构守卫 | `tests/architecture/`, `CMakeLists.txt`, `include/cch/` | `include/cch/` 仅保留 domain contract 与抽象 seam；实现细节在 `src/`（CMake `PRIVATE` include）。改公共 header、依赖或 include surface 时必须跑 architecture tests。 |
