...
| Slash 命令 / Prompt 处理 | `include/cch/coding_agent/PromptProcessing.hpp`, `src/coding_agent/PromptProcessing.cpp`, `src/coding_agent/PromptExpander.cpp`, `tests/coding_agent/BuiltinCommandsTest.cpp`, `tests/coding_agent/PromptExpanderTest.cpp` | `CommandRegistry` 管理 session-lifecycle 内建命令（`/help`, `/clear`, `/compact`, `/exit`）；`expand_prompt_template()` 支持 bash-style 参数替换；`process_prompt()` 缝合 REPL/runner/RPC。 |
...
| Session JSONL / resume | `include/cch/harness/session/`, `src/harness/session/JsonlSessionStore.cpp`, `tests/harness/session/` | JSONL 脱敏 typed transcript；v2 message resume 稳定；v3 tree metadata parse-only。 |
