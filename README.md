...
- `cch_coding_agent_interactive` (`src/coding_agent/tui/InteractiveMode.*`): the private Native TUI composition that connects a real Agent Session to the reusable TUI, Editor, themes, and effective keybindings. It submits prompts asynchronously, reduces cumulative lifecycle message state into one coherent transcript, and restores Virtual or Process Terminals on exit. It is exercised directly by integration tests but is not selected by the public CLI before #64; SDK, text, JSON, and RPC paths do not link it.
...
The exact immutable effective registry drives component dispatch, hotkey help, and rendered key hints. Linux/macOS job-control defaults and native Windows unavailability are explicit when an application action is concretely registered. See [Native TUI keybindings](docs/keybindings.md) for the grammar, implemented IDs, precedence, diagnostics, platform behavior, and pi baseline sources. This slice does not add a production TUI mode or hot reload.
