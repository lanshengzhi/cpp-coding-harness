...
User-level state lives in the agent config directory `~/.cpp-harness/agent/`, mirroring pi's `~/.pi/agent/`: `auth.json` (static API keys), `settings.json` (provider defaults, theme selection, and resource policy), `trust.json` (persisted project trust decisions), `themes/` (global Native TUI themes), and `sessions/<encoded-workspace>/` (default persisted Agent Session histories). Set `CCH_CODING_AGENT_DIR` to override the location (pi: `PI_CODING_AGENT_DIR`). These paths are centralized in `include/cch/coding_agent/AgentConfigDir.hpp`; theme discovery never falls back to `~/.pi`.
...
- `cch_coding_agent_tui` (`src/coding_agent/tui`): the physically separate coding-agent Native TUI presentation layer. Its current supported slice provides baseline-pinned built-in dark/light palettes, pi-compatible theme-file parsing, deterministic global/trusted-project/explicit discovery and precedence, bounded diagnostics, appearance-aware selection, capability-aware color mapping, late-bound adapters, and a Theme settings overlay that invalidates the complete TUI after selection. Discovery is a one-shot startup operation with no generalized hot reload. Production frontend assembly remains deferred; this target is not linked into the non-interactive runtime.
