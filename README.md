...
- `cch_coding_agent_tui` (`src/coding_agent/tui`): the physically separate coding-agent Native TUI presentation layer. Its current supported slice provides baseline-pinned built-in dark/light palettes, explicit pi-compatible theme-file parsing, bounded diagnostics, capability-aware color mapping, and late-bound adapters for reusable TUI styling. Theme discovery, settings selection, and production frontend assembly remain deferred; this target is not linked into the non-interactive runtime.
