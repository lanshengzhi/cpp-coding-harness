...
The registry owns tool capabilities directly. In default sequential execution, every call starts its Tool Execution lifecycle before the executor parses and clones the raw arguments, applies the foundational primitive and recursive-object coercion profile, and validates the tool's JSON Schema before policy hooks or capability invocation. Malformed JSON, unknown tools, invalid schemas, unsupported executable keywords, and schema-invalid values become bounded error Tool Call Outcomes for only their calls; annotations and extension members do not disable recognized assertions.
