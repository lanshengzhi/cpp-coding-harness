...
- One persistent event-subscription path via move-only `agent::AgentEventSink`; prompt progress is not returned or delivered through per-prompt sinks. A second session-event subscription (`subscribe_session`, pi `AgentSessionEvent`) delivers the turn auto-retry `auto_retry_start`/`auto_retry_end` events through the same weak-observer semantics (T12).
